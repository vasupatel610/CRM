...
# Configuration
# Increased number of transactions and customers for richer data
num_transactions = 5000  # Was 1000, now 5 times more
max_lines_per_transaction = 5
num_customers = 1000  # Was 300, now more customers
num_products = 50
num_branches = 10
channels = ["Online", "In-Store", "B2B"]
payment_modes = ["Cash", "Card", "BNPL"]

# Electronic products list (50 products) - No change needed here for visualization
electronic_products = [
...
# Product category mapping - No change needed here for visualization
...
# Generate mapping tables - Increased customer_ids
...
# Base price mapping - No significant change, ranges seem reasonable
...
# Monthly weights adjusted to make some months clearly higher/lower for sales trends
month_weights = {
    1: 0.8, 2: 0.7, 3: 1.0, 4: 1.1, 5: 0.9, 6: 1.3, # June, July, Oct, Nov, Dec higher
    7: 1.4, 8: 1.0, 9: 1.2, 10: 1.5, 11: 1.7, 12: 2.0 # Increased peak months for clearer seasonality
}
current_date = datetime.now().date()
date_pool = []
# Generating data for 3 years to show longer trends
for year in [2023, 2024, 2025]:
    for month, weight in month_weights.items():
        # Increased range for date pool to accommodate more transactions
        for _ in range(int(weight * 300)): # Increased multiplier for more dates
...
channel_quantity_weights = {
    "Online": [0.6, 0.3, 0.08, 0.02], # More 1-item purchases
    "B2B": [0.3, 0.4, 0.25, 0.05],   # More multi-item purchases
    "In-Store": [0.4, 0.35, 0.2, 0.05] # Balanced
}

# Generate customer ages - No change needed
...
for _ in range(num_transactions):
    ...
    # Adjusted probabilities for voucher redemption and upsell to create more varied data
    voucher_redeemed_flag = np.random.choice(["Yes", "No"], p=[0.25, 0.75]) # 25% voucher redemption
    is_upsell = np.random.choice(["Yes", "No"], p=[0.3, 0.7]) # 30% upsell
    is_returned = "No"
    ...
    for line_num in range(1, num_lines + 1):
        ...
        # Slightly adjusted discount rates to show more impact
        discount_rate_on_line = np.random.choice([0, 0.02, 0.05, 0.1, 0.15], p=[0.6, 0.15, 0.1, 0.1, 0.05]) # Added 15% discount
...
# Add returned flags - Adjusted return rate to be slightly higher and more variable
for month in df["month"].unique():
    month_idx = df[df["month"] == month].index
    rate = random.uniform(0.08, 0.18) # Was 0.05-0.14, now higher
...
# Monthly targets - Adjusted multipliers to create clearer target vs actual gaps
monthly_target_multipliers = {
    1: 1.10, 2: 1.15, 3: 1.05, 4: 1.08, 5: 1.12, 6: 0.95, # Some months slightly below target
    7: 0.98, 8: 1.02, 9: 1.00, 10: 1.04, 11: 1.06, 12: 1.07 # Others slightly above
}
df["target_multiplier"] = df["month"].map(monthly_target_multipliers)

# Add geolocation - No change, seems reasonable
random.seed(42)
...
# Define churn probability based on recency, returns, and age - Adjusted for clearer churn segments
def get_churn_probability(row):
    prob = 0.05 # Base churn probability
    if row['recency'] > 250: # Very high recency, increased impact
        prob += 0.35
    elif row['recency'] > 120: # High recency
        prob += 0.20
    elif row['recency'] > 60: # Medium recency
        prob += 0.10
    
    if row['total_returns'] >= 5: # Very many returns, high impact
        prob += 0.30
    elif row['total_returns'] >= 2: # Some returns
        prob += 0.15
        
    if row['customer_age'] >= 65: # Older customers slightly higher churn
        prob += 0.10
    elif row['customer_age'] <= 22: # Younger customers slightly higher churn
        prob += 0.07
    
    return min(prob, 0.9) # Increased cap probability to 90% to allow for higher risk
...
social_media_platforms = ["Twitter", "Facebook", "Instagram", "TikTok", "LinkedIn", "YouTube"] # Added YouTube
hashtags_choices = ["#electronics", "#newtech", "#gadgetreview", "#productlove", "#techdeals", "#unboxing", "#smartlife", "#innovate"] # More diverse hashtags
campaign_names = ["HolidayTechSale", "SpringSavings", "GamingGearLaunch", "WorkFromHomeEssentials", "BackToSchoolTech"] # More specific campaigns

# Enhanced long review samples with detailed feedback (no change, already good)
...
# Dynamic phrase components for creating varied reviews (no change, good as is)
intro_phrases = [
...
# Generate reviews for a subset of actual purchases - Increased review probability
review_probability = 0.60   # Was 0.45, now 60% of purchases get reviewed
purchases_to_review = customer_product_purchases.sample(n=int(len(customer_product_purchases) * review_probability), random_state=42) # Added random_state for reproducibility
...
for _, purchase in purchases_to_review.iterrows():
    ...
    # Sentiment bias based on return status and some randomness - Adjusted weights for clearer patterns
    if is_returned == "Yes":
        # Returned items more likely to have negative reviews
        sentiment_weights = [0.05, 0.85, 0.1]   # [positive, negative, neutral] - Higher negative
    else:
        # Non-returned items more likely to have positive reviews
        sentiment_weights = [0.75, 0.05, 0.20]  # [positive, negative, neutral] - Higher positive, lower negative
    ...
    hashtags = ' '.join(random.sample(hashtags_choices, random.randint(2,4))) # Allow up to 4 hashtags
...
funnel_stages = ["Lead", "Awareness", "Consideration", "Purchase", "Service", "Loyalty", "Advocacy"] # Added 'Advocacy'
num_journeys = 2500 # Increased number of journeys for more data points
journey_data = []
...
for _ in range(num_journeys):
    ...
    # Simulate funnel drop-offs with higher initial drop-off, more distinct stages
    # Adjusted weights to show clearer drop-off, but also more reaching 'Purchase' and 'Loyalty'
    drop_stage_weights = [0.25, 0.20, 0.18, 0.15, 0.10, 0.07, 0.05] # Total 1.0 (5% reach advocacy)
    # The 'None' implies completion of all stages up to the defined funnel.
    # The remaining probability (1 - sum of above) implicitly means they complete the whole journey.
    
    # Choose the stage where the customer drops off (or completes all defined stages)
    # Using random.choices directly will select from the list. If 'Advocacy' is reached, it means they didn't drop off before it.
    # We need to map this to the *last* stage they reached before dropping off.
    
    # A better way to model this is to decide the *last stage reached*.
    # Higher chance of reaching early stages, decreasing chance for later stages.
    ...
    for i, prob in enumerate(cumulative_probs):
        if r < prob:
            ...
            break
    if final_stage_reached is None: # If no drop-off (r >= last cumulative prob), they complete all stages.
        final_stage_reached = funnel_stages[-1] # Set to the last defined stage
    ...
    for i, stage in enumerate(funnel_stages):
        if final_stage_reached is not None and funnel_stages.index(stage) > funnel_stages.index(final_stage_reached):
            break # Customer dropped off at or before final_stage_reached

        entry_date = (base_date + timedelta(days=random.randint(5,15)*i, hours=random.randint(0,23), minutes=random.randint(0,59))).strftime("%Y-%m-%d %H:%M:%S") # Increased time between stages for realism

        # Simulate campaign opens and clicks based on early stages and overall campaign effectiveness
        # Adjusted rates to make opens/clicks more distinct from conversion
        if stage == "Lead":
            if random.random() < 0.80: # 80% open rate for leads (higher)
                campaign_open = "Yes"
            if campaign_open == "Yes" and random.random() < 0.35: # 35% click rate for opened emails (lower than open)
                campaign_click = "Yes"
        
        # Conversion typically happens at or around the 'Purchase' stage
        if stage == "Purchase":
            # Higher chance of conversion if they reach this stage
            if random.random() < 0.6: # 60% chance to convert at purchase stage
                conversion_flag = "Yes"
            else:
                conversion_flag = "No" # If they reach purchase but don't convert
        elif stage in ["Service", "Loyalty", "Advocacy"]:
            # If they pass purchase, assume conversion has happened
            conversion_flag = "Yes"
